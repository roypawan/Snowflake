...
CREATE STAGE VIN.DECODE.aws_s3_bucket 
  URL = 's3://uni-cmcw/'
  COMMENT = 'External S3 stage for uni-cmcw';

DESC STAGE VIN.DECODE.aws_s3_bucket;
...
select count(*) from vin.decode.MAKE_MODEL_VDS;
...
select get_ddl('function','VIN.DECODE.PARSE_AND_ENHANCE_VIN(VARCHAR)');

SHOW USER FUNCTIONS IN SCHEMA VIN.DECODE;
